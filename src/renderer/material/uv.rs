//! UV coordinate visualization material for debugging
//!
//! Renders derived UV coordinates as color: U = Red, V = Green.
...
/// Material that visualizes UV coordinates as colors.
...
impl UVMaterial {
    ...
    pub fn new(ctx: &WgpuContext, format: wgpu::TextureFormat) -> anyhow::Result<Self> {
        ...
        let model_bind_group_layout =
            ctx.device
                .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                    label: Some("uv model bind group layout"),
                    entries: &[wgpu::BindGroupLayoutEntry {
                        binding: 0,
                        visibility: wgpu::ShaderStages::VERTEX,
                        ty: wgpu::BindingType::Buffer {
        ...
        let camera_buffer = RawUniformBuffer::new(
            ...
            Some("uv camera uniform"),
        );

        let camera_bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("uv camera bind group"),
